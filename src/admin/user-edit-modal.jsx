...
import { X, Pencil, AlertCircle, CheckCircle } from "lucide-react"
...
export default function UserEditModal({ isOpen, onClose, user, onUserUpdated }) {
  ...
  useEffect(() => {
    if (isOpen && user) {
      setFormData({
        ...
        situation: user.situation || "aucune",
        region: user.region || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await userApiService.updateUser(user.id, formData)
      ...
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          ...
          <div className="bg-gray-50 p-4 rounded-lg mb-6">
            <h3 className="font-semibold text-gray-700 mb-2">Informations de base</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 font-medium">{new Date(user.created_at).toLocaleDateString("fr-FR")}</span>
              </div>
              <div>
                <span className="text-gray-600">Dernière connexion:</span>
                <span className="ml-2 font-medium">{user.last_login_formatted}</span>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center">
                <input
                  ...
                  disabled={updateStatus.isLoading}
                />
                <label htmlFor="edit_is_admin" className="ml-2 text-gray-700">
                  Administrateur
                </label>
              </div>
              <div className="flex items-center">
                <input
                  ...
                  disabled={updateStatus.isLoading}
                />
                <label htmlFor="edit_email_notifications" className="ml-2 text-gray-700">
              ...
              <div className="flex items-center">
                <input
                  type="checkbox"
                  id="edit_sms_notifications"
                  checked={formData.sms_notifications}
                  onChange={(e) => handleInputChange("sms_notifications", e.target.checked)}
                  className="rounded border-gray-300 text-orange-600 focus:ring-orange-500"
                  disabled={updateStatus.isLoading}
                />
                <label htmlFor="edit_sms_notifications" className="ml-2 text-gray-700">
            ...
            {updateStatus.error && (
              <div className="flex items-center gap-2 p-4 bg-red-50 border border-red-200 rounded-lg text-red-700">
                ...
                <div className="whitespace-pre-line">{updateStatus.error}</div>
              </div>
            )}
            ...
            {updateStatus.success && (
              <div className="flex items-center gap-2 p-4 bg-green-50 border border-green-200 rounded-lg text-green-700">
                ...
                <div>Utilisateur modifié avec succès !</div>
              </div>
            )}
            ...
            <div className="flex gap-4 justify-end pt-4 border-t">
              ...
              <button
                type="submit"
                disabled={updateStatus.isLoading || updateStatus.success}
